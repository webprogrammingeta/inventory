          ...
          <div class="col-12">
            <div class="card card-outline card-dark bg-dark">
              <div class="card-header bg-warning">
                <h1 class="card-title">Tabel Data Usulan Pengadaan</h1>

               <a href="cetak_usulan.php" type="button" class="btn bg-dark float-right"  target="_blank">
                       <span class="text-light">Print Usulan Pengadaan</span>
              ...
              <?php foreach ($rows as $row) : ?>
                  <tr>
                   <td><?= $i++; ?></td>
                   <td><span class="badge badge-light text-md"><?= $row['id_usulan'] ?></span></td>
                   <td><?= $row['komisi'] ?></td>
                  ...
                  <td>Rp.<?= number_format($row['harga'], 0, ',', '.'); ?></td>
                   ...
                   <td >
                       <a href="accept_usulan.php?id=<?= $row['id'] ?>" class="btn  btn-success btn-app-sm mr-1 mb-3">
                        Konfirmasi ✓
                        </a>  
                        <a href="reject_usulan.php?id=<?= $row['id'] ?>" class="btn  btn-danger btn-app-sm mb-3">
                        Tolak ✕
                        </a>  
                    </td>
                  </tr>
      ...
      const flashubah = $('.flash-ubah').data('flashubah')
        if (flashubah) {
          Swal.fire({
             type: 'success',
             title: 'Sukses',
             text: 'Data usulan berhasil dikonfirmasi',
             showConfirmButton: false,
             timer: 1200
          }).then(function(){ 
      ...
      const flashgagal = $('.flash-gagal').data('flashgagal')
        if (flashgagal) {
          Swal.fire({
             type: 'success',
             title: 'Sukses',
             text: 'Data usulan berhasil ditolak',
             showConfirmButton: false,
             timer: 1400
          }).then(function(){ 
