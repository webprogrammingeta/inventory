...
include 'templates/header.php';
    ...
    $date = date("Ymd");
    $query = mysqli_query($koneksi, "SELECT max(kd_usulan_penghapusan) as kodeTerbesar FROM tbl_usulan_penghapusan");
...
<?php 
     if(isset($_POST['submit'])){
        ...
        $c = $_POST['namabarang'];
        $d = $_POST['ketusulan'];
        $query = "INSERT INTO `tbl_usulan_penghapusan`(`id`,`kd_usulan_penghapusan`,`kode_barang`,`nama_barang`,`keterangan_usulan`,`status_usulan_penghapusan`)VALUES(NULL,'$a','$b','$c','$d','pending')";
        if(performQuery($query)){
            // echo "<script>alert('Data berhasil disimpan.')'</script>";
            echo "<script>document.location='penghapusan.php'</script>";
        }else{
          ...
          <form class="needs-validation" name="Ftambah" method="POST" autocomplete="off" enctype="multipart/form-data">
            ...
            <div class="row">
              ...
              <div class="col-md-4 mb-3">
               ...
               <div class="col-md-4 mb-3">
                <label for="barang" class="input-group-text">Nama Barang</label>
                <select name="namabarang" id="namabarang" class="form-control" onchange='changeValue(this.value)' required>  
                   <?php   
                             ...
                             $query = mysqli_query($con, "select * from tbl_barang order by nama_barang desc");  
                              $result = mysqli_query($con, "select * from tbl_barang");  
                              $a          = "var kode = new Array();\n;";  
                              while ($row = mysqli_fetch_array($result)) {  
                               echo '<option name="namabarang" value="'.$row['nama_barang'] . '">' . $row['nama_barang'] . '</option>';   
                          
                              $a.= "kode['" . $row['nama_barang'] . "'] = {kode:'" . addslashes($row['kd_barang'])."'};\n";  
                              }  
                              ?>  
                 </select>
            </div>
               ...
               <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control" name="kdbarang" id="kdbarang" value=""   readonly="">
              </div>
            ...
            <div class="col-md-12 mb-3">
              ...
              <div class="card-body">
                <table id="example1"class="table table-bordered table-striped  table-hover text-center table-secondary text-dark">
                  ...
                  <tbody>
    <?php $i = 1;
        $rows = mysqli_query($koneksi, "SELECT * FROM tbl_usulan_penghapusan ORDER BY id ASC")?>
              <?php foreach ($rows as $row) : ?>
                  <tr>
                   <td><?= $i++; ?></td>
                   <td><span class="badge badge-light text-md">
                   <?= $row['kd_usulan_penghapusan'] ?></span></td>
                   <td><?= $row['kode_barang'] ?></td> 
                   <td align="left"><?= $row['keterangan_usulan'] ?></td>
                   <td><span class="badge badge-success">
                   <?= $row['status_usulan_penghapusan'] ?></span></td>
                  <!--  <td>
  ...
  <script>
         <?php  echo $a; ?>  
          function changeValue(id){  
            document.getElementById('kdbarang').value = kode[id].kode;
          };  
  </script>
      ...
      const flashdata = $('.flash-data').data('flashdata')
        if (flashdata) {
          Swal.fire({
            ...
            window.location.assign("http://localhost:8080/kalvarisarpas/operator/penghapusan.php");
            });
        }
