...
<?php 
     if(isset($_POST['submit'])){
        ...
        $f = $_POST['peminjam'];
        $g = $_POST['telp'];
        $query = "INSERT INTO `tbl_pengajuan_peminjaman` (`id`,`id_pengajuan`,`kd_barang`,`nama_barang`,`harga`,`keterangan_peminjaman`,`nama_peminjam`,`no_telp`,`status`,`tanggal_pengajuan`)VALUES(NULL,'$a','$b','$c','$d','$e','$f','$g','pending',CURRENT_TIMESTAMP)";
        if(performQuery($query)){
            // echo "<script>alert('Data berhasil disimpan.')'</script>";
            echo "<script>document.location='index.php?=success'</script>";
        }else{
          ...
          <div class="col-12">
            <div class="card card-outline card-dark bg-dark">
              <div class="card-header bg-warning">
                <h4 class="card-title">Tabel Pencarian Barang</h4>
          ...
          <form class="needs-validation" name="Ftambah" method="POST" autocomplete="off" enctype="multipart/form-data">
            ...
            <div class="row">
              ...
              <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control" name="idpeminjaman" id="idpeminjaman" value="<?= $idpeminjaman; ?>" placeholder="" readonly="" required="">
              </div>

                <input type="hidden" class="form-control" name="peminjam" id="peminjam" value="<?= $nama; ?>" placeholder="" readonly="" required="">
              ...
              <div class="col-md-4 mb-3">
               ...
               <div class="col-md-4 mb-3">
                ...
                <input type="number" class="form-control" name="telp" id="telp" placeholder="" required="">
              </div>
               <div class="col-md-4 mb-3">
                <label class="input-group-text">
    ...
    <?php $i = 1;
        $rows = mysqli_query($koneksi, "SELECT * FROM tbl_barang ORDER BY id ASC")?>
              <?php foreach ($rows as $row) : ?>
                  <tr>
                   <td><?= $i++; ?></td>
                   <td><img src="../operator/barang/<?= $row["foto"]; ?>" width="70" title="<?= $row['foto']; ?>"></td>
    ...
    <?php endforeach; ?>
      ...
      <?php if (isset($_GET['delete'])) : ?>
          ...
          <?php if (isset($_GET['success'])) : ?>
            <div class="flash-ubah" data-flashubah="<?= $_GET['success']; ?>"></div>
          <?php endif; ?>
...
<script type="text/javascript" src="<?= $base_url; ?>assets/plugins1/jquery-3.4.1.min.js"></script>
  ...
  $("#simpan").click(function(){
        ...
        var nmpj = $("#peminjam").val();
        var ket = $("#ketpeng").val();
        if ( id == ''|| nama == '' || kategori == '' || harga == ''|| nmpj == ''|| ket == '' ) {
            Swal.fire({
                title: " Peringatan",
                type: "warning",
                icon: "warning",
      ...
      const flashubah = $('.flash-ubah').data('flashubah')
        if (flashubah) {
          Swal.fire({
             type: 'success',
             title: 'Sukses',
             text: 'Data Peminjaman sedang diproses !',
             showConfirmButton: false,
             timer: 2000
          }).then(function(){ 
            window.location.assign("http://localhost:8080/sarpas/operator/peminjaman.php");
