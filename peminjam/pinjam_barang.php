<?php
    ...
    if(count(fetchAll($query)) > 0){
        foreach(fetchAll($query) as $row){
            ...
            // INSERT PEMINJAMAN
            $query = "INSERT INTO `tbl_peminjaman`(`id`,`id_peminjaman`,`kd_barang`,`nama_barang`,`jumlah`,`nama_peminjam`,`no_telp`,`keterangan_peminjaman`,`status`,`tanggal_peminjaman`)VALUES(NULL, '$a', '$b','$g','$c','$d','$e','$f','sedang dipinjam',CURRENT_TIMESTAMP);";
        }
            // UPDATE PENGAJUAN
        $query .= "UPDATE `tbl_pengajuan_peminjaman` SET status = 'sedang dipinjam' WHERE `tbl_pengajuan_peminjaman`.`id` = '$id';";
